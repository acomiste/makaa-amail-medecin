...
# Force Python à vider son buffer d'affichage immédiatement pour voir les logs en direct sur GitHub
sys.stdout.reconfigure(line_buffering=True)

FICHIER_SOURCE = "medecins.csv"
FICHIER_RESULTAT = "resultats_medecins.csv"

NB_THREADS = 2          # Nombre de navigateurs Chrome simulés en parallèle
...
# True = Mode invisible obligatoire pour GitHub Actions (sans écran externe)
MODE_HEADLESS = True
...
verrou_demarrage_driver = threading.Lock()  # Évite les conflits au lancement des instances
...
def initialiser_fichiers():
    if not os.path.exists(FICHIER_SOURCE):
        print(f"❌ Erreur : Le fichier '{FICHIER_SOURCE}' est introuvable dans le dépôt GitHub.", flush=True)
        return False
...
def lire_csv_avec_fallback_encodage(chemin_fichier):
    ...
    for enc in encodages_a_tester:
        try:
            with open(chemin_fichier, mode='r', encoding=enc, newline='') as f:
                ...
                lignes = list(reader)
            print(f"📖 Fichier '{chemin_fichier}' lu avec succès (Encodage : {enc})", flush=True)
    ...
    raise UnicodeDecodeError(
        "utf-8", b"", 0, 1,
        f"Impossible de lire {chemin_fichier} sur GitHub avec les encodages testés : {derniere_erreur}"
    )
...
def sauvegarder_source_de_maniere_sure(champs, lignes_medecins):
    with verrou_fichier:
        ...
        try:
            ...
            print(f"⚠️ Erreur lors de la mise à jour de la source sur le serveur : {e}", flush=True)
...
def attendre_si_pause_en_cours():
    ...
    if maintenant < pause_jusqu_a:
        ...
        print(f"⏸ [Pause globale] {int(attente)}s restantes dans le cloud...", flush=True)
        time.sleep(attente)
...
def signaler_medecin_traite(nom_thread):
    with verrou_etat:
        ...
        if compteur_actuel % PAUSE_TOUS_LES_N == 0:
            ...
            print(f"\n🛑 [{nom_thread}] {compteur_actuel} médecins traités. Pause de sécurité globale lancée...\n", flush=True)
...
def creer_driver():
    ...
    if MODE_HEADLESS:
        ...
        options.add_argument("--remote-debugging-pipe")  # Prévient les freezes de Chrome sous Linux
    ...
    try:
        ...
        for mot in mots_cles_bloquants:
            if mot in html_page or mot in url_actuelle:
                print(f"\n╔════════════════════════════════════════════════════════════╗", flush=True)
                print(f"  🛑 [{nom_thread}] [ALERTE SÉCURITÉ : BLOCAGE ROBOT DÉTECTÉ] 🛑", flush=True)
                print(f"  ⚠️  Le script a rencontré un Captcha pour Dr {prenom} {nom}", flush=True)
...
def collecter_liens_visuels_ddg(driver):
    liens_trouves = []
    selectors = ["[data-testid='result-title-a']", ".result__url", "a"]
    for sel in selectors:
        try:
            elements = driver.find_elements(By.CSS_SELECTOR, sel)
            for elem in elements:
                href = elem.get_attribute("href")
                if href and "duckduckgo.com/?q=" not in href and href.startswith("http") and href not in liens_trouves:
...
def traiter_medecin(driver, medecin, cle_prenom, cle_nom, nom_thread):
    ...
    print(f"🚀 [{nom_thread}] Traitement : {prenom} {nom}...", flush=True)

    mots_cles = "cpts msp sisa thèse"
    requete_complete = f'"{prenom}" "{nom}" {mots_cles}'
    url_initiale = f"https://duckduckgo.com/?q={quote(requete_complete)}"
    urls_a_visiter = []
    ...
    try:
        ...
        urls_a_visiter.extend(collecter_liens_visuels_ddg(driver))
    except Exception as e:
        print(f"    ⚠️ [{nom_thread}] Problème lors du chargement de DuckDuckGo : {e}", flush=True)
    ...
    if urls_a_visiter:
        print(f"[{nom_thread}] -> {len(urls_a_visiter)} site(s) web détecté(s). Analyse...", flush=True)
        ...
        print(f"    ℹ️ [{nom_thread}] Aucun résultat externe extrait pour ce médecin.", flush=True)
    ...
    try:
        ...
        while True:
            ...
            if not medecin_a_traiter:
                print(f"🏁 [{nom_thread}] Plus aucun médecin à traiter dans la liste.", flush=True)
                break
                
            try:
                traiter_medecin(driver, medecin_a_traiter, cle_prenom, cle_nom, nom_thread)
            except Exception as e:
                print(f"❌ [{nom_thread}] Erreur sur la ligne en cours : {e}", flush=True)
...
if __name__ == "__main__":
    ...
    # DETECTION STRICTE DES EN-TÊTES DE COLONNES CORRECTES :
    cle_prenom = next((c for c in champs if 'prénom' in c.lower() or 'prenom' in c.lower()), None)
    cle_nom = next((c for c in champs if 'nom' in c.lower() and 'prénom' not in c.lower() and 'prenom' not in c.lower()), None)

    if not cle_prenom or not cle_nom:
        print(f"❌ Erreur : Colonnes 'Prénom' ou 'Nom' introuvables. Colonnes lues : {champs}", flush=True)
        exit(1)
    ...
    print(f"🔥 {len(lignes_filtrées)} médecin(s) restant(s) à extraire dans le cloud.", flush=True)
    ...
    for i in range(NB_THREADS):
        ...
        t.start()
        time.sleep(3.0)  # Temporisation augmentée pour stabiliser le réseau Linux au démarrage

    for t in threads_actifs:
        t.join()

    print("\n🏁 [FIN DU PROCESSUS] Exécution terminée sur GitHub Actions.", flush=True)
